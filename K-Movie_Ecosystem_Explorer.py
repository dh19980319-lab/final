...
def analyze_hitmaker_index(movie_records, entity_type='Director', index_type='Total_Audience'):
    """
    감독 또는 회사의 흥행 기여도를 분석합니다. (Total_Audience 또는 Avg_Audience 기준)
    """
    ...
    for name, data in entity_data.items():
        ...
        if total_aud > 0:
            avg_aud = total_aud / data['movie_count'] if data['movie_count'] > 0 else 0
            
            sort_index = total_aud if index_type == 'Total_Audience' else avg_aud
            ...
            results.append({
                ...
                'Avg_Audience': int(avg_aud), # 평균 관객 수 지표 추가
    ...
    df['Avg_Audience_Formatted'] = df['Avg_Audience'].apply(lambda x: f"{x:,.0f} 명") # 포맷팅 추가
...
def main():
    ...
    st.markdown("<h3 style='color:#4299e1;'>데이터 기반으로 영화 산업의 성공 요소를 분석하세요.</h3>", unsafe_allow_html=True)
    st.markdown("---")
                ...
                if not analysis_df.empty:
                    ...
                    for index, row in top_df.iterrows():
                        ...
                        with st.expander(f"**#{index}: {name} ({row['Movie_Count']}편)**", expanded=False):
                            st.markdown("- " + "\n- ".join(movie_list) if movie_list else "분석 기간 내 흥행 기록이 있는 영화가 없습니다.")
                else:
